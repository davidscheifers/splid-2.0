...
CREATE TABLE IF NOT EXISTS evide."Group"
(
...
CREATE TABLE IF NOT EXISTS evide."Group_User"
(
    username character varying(255) COLLATE pg_catalog."default" NOT NULL,
    "group_id" uuid NOT NULL,
    CONSTRAINT "Group_User_pkey" PRIMARY KEY (username, "group_id"),
    CONSTRAINT "group" FOREIGN KEY ("group_id")
        REFERENCES evide."Group" (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
        NOT VALID,
    CONSTRAINT "user" FOREIGN KEY (username)
        REFERENCES evide."User" (username) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
);

CREATE TABLE IF NOT EXISTS evide."Bill"
(
    id uuid NOT NULL,
...
CREATE TABLE IF NOT EXISTS evide."Transaction"
(
    id uuid NOT NULL,
    description character varying(255) COLLATE pg_catalog."default",
    ...
    CONSTRAINT "bill" FOREIGN KEY (bill_id)
        REFERENCES evide."Bill" (id) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION
);

CREATE TABLE IF NOT EXISTS evide."Accounting"
(
    username character varying(255) COLLATE pg_catalog."default" NOT NULL,
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        IF NOT EXISTS (SELECT 1 FROM evide."Accounting" WHERE username = NEW.receiver_username) THEN
            INSERT INTO evide."Accounting"(username, group_id, balance) VALUES (NEW.receiver_username, NEW.group_id, NEW.amount);
        ELSE
            UPDATE evide."Accounting" SET balance = balance + NEW.amount WHERE username = NEW.receiver_username AND group_id = NEW.group_id;
        END IF;
    ELSIF TG_OP = 'UPDATE' THEN
        UPDATE evide."Accounting" SET balance = balance + NEW.amount - OLD.amount WHERE username = NEW.receiver_username AND group_id = NEW.group_id;
    ELSIF TG_OP = 'DELETE' THEN
        UPDATE evide."Accounting" SET balance = balance - OLD.amount WHERE username = OLD.receiver_username AND group_id = NEW.group_id;
    END IF;
    RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
AFTER INSERT ON evide."Group_User"
FOR EACH ROW
...
BEGIN
    UPDATE evide."Group"
    SET "update_at" = NOW()
    FROM evide."Transaction"
    WHERE evide."Group".id = NEW."group_id";
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
